// Server-side AI engine: generation, semantic + lexical dedup, and evaluation.
//
// This module is imported by BOTH api/ai.ts (the admin endpoint) and
// api/cron.ts (the daily job). It deliberately lives under src/services rather
// than api/ — Vercel turns every file in api/ into its own serverless function,
// and one api/ route importing another (api/cron -> api/ai) crashes at runtime
// (FUNCTION_INVOCATION_FAILED). Importing a plain module from src/ is supported.
//
// Server-only: reads process.env and calls fetch. Never imported by client code,
